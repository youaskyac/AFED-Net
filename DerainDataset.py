...
def Im2Patch(img, win, stride=1):
    ...
    for i in range(win):
        for j in range(win):
            ...
            Y[:, k, :] = np.array(patch[:]).reshape(endc, TotalPatNum)  # 展开成一列
            k = k + 1
...
def prepare_data_Rain12600(data_path, patch_size, stride):
    ...
    train_num = 0
    for i in range(900):  # 选取900张图片进行训练
        target_file = "%d.jpg" % (i + 1)  # 标签文件
        target = cv2.imread(os.path.join(target_path, target_file))
        b, g, r = cv2.split(target)  # 拆分通道
        ...
        for j in range(14):
            ...
            target_patches = Im2Patch(target_img.transpose(2, 0, 1), win=patch_size, stride=stride)  # 转换格式和尺寸大小
